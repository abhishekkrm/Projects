...
class PrintScheduler:
    ...
    def __init__(self, num_jobs):
        ...
        self.printer_avail = Semaphore(0)
        self.job_avail = Semaphore(0)
        self.cur_jobs = 0
        self.cur_printer = 0
        self.max_job = num_jobs
        pass
    ...
    def printer_ready(self):
        ...
        if self.cur_jobs > 0:
            self.cur_jobs -=1
            self.mutex.release()
            self.job_avail.release()
        else:
            self.cur_printer = self.cur_printer + 1
            self.mutex.release()
            self.printer_avail.acquire()
        pass
    ...
    def submit_job(self):
        ...
        self.mutex.acquire()
        if self.cur_jobs == self.max_job:
            self.mutex.release()
            raise
        if self.cur_printer > 0:
            self.cur_printer -=1
            self.mutex.release()
            self.printer_avail.release()
        else:
              self.cur_jobs +=1
              self.mutex.release()
              self.job_avail.acquire()
        pass
