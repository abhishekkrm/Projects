...
# the segment manager manages the current segment, flushing it
...
class SegmentManagerClass:
    ...
    def write_to_newblock(self, data):
        ...
        # if -1 then error check next segment 
        if -1 == blockno:
            # FLush the segment in memory
            self.flush()
            for _ in range(0, NUMSEGMENTS):
                self.segcounter =(1+self.segcounter)% NUMSEGMENTS 
                self.currentseg = SegmentClass(self.segcounter)
                blockno = self.currentseg.write_to_newblock(data)
                if blockno != -1:
                    break
        ...
        return self.currentseg.segmentbase + blockno
        pass
