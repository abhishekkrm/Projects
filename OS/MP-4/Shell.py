...
class Shell:
    ...
    def create(self, args):
        if len(args) != 3:
            ...
            return
        if not self.is_number(args[2]):
            print "Invalid file length, Usage: create filename length"
            return 
    ...
    def rm(self, args):
        # We have to check if the args are valid!!
        if len(args) != 2:
        ...
        if isdir:
            raise FileSystemException("Cannot remove: %s, use rmdir dirname or provide a valid filename" % filename)
        else :
            LFS.filesystem.unlink(filename)
        pass
    ...
    def rmdir(self, args):
        ...
        if isdir:
            if size>0:
                raise FileSystemException("Directory not empty %s " % dirname)
            else:
                LFS.filesystem.unlink(dirname)
        else:
            raise FileSystemException("Cannot remove: %s, use rm filename or provide a valid dirname" % dirname)
        pass
